...
:::important[重要提醒]
推荐使用大陆外的 **Ubuntu 24.04 LTS** 服务器。    
如果没有服务器，使用自己的电脑也可以。使用国内服务器的话，请确保域名已经备案。使用自己的域名的话请务必确保**域名能正确访问到你的服务器**，并且80和443端口正常开放
:::
...
申请证书（无cdn情况）
```bash
# 为所有子域名申请证书，需要把"yourdomain.com"替换为你自己的域名
...
<details>
<summary>DNS-01方式申请</summary>

DNS-01 验证方式相比 HTTP-01 有以下优势：
- ✅ **支持泛域名证书**（*.example.com）
- ✅ **无需开放80端口**，适合内网服务器
- ✅ **不受防火墙限制**
- ✅ **验证更可靠**
...
> ⚠️ **替换 `YOUR_API_TOKEN_HERE` 为你之前获取的 API Token**
...
Let's Encrypt 证书有效期为 **90天**，Certbot 支持自动续期。
...
> [!WARNING]
> 这一步有坑！！！阿里云AccessKey填写正确之后发不出邮件，后端显示`Aliyun.Acs.Core.Exceptions.ClientException: InvalidMailAddress.NotFound : The specified mailAddress does not exist.`请注意需要使用**新加坡站点**的发件邮箱! 在DM控制台左上角下拉框改成新加坡重新配置发件域名和发件地址！

> [!WARNING]
> 这一步有坑！！！如果你前面没有在源码修改前端面板地址，就需要前往数据库手动修改！否则管理面板会无法登录！
> ```
> UPDATE system_options SET value = '<website前端的地址，不要带最后的斜杠！>' WHERE `key` = 'ProjectFrontendPrefix';
> ```
> **修改后，需要重启backend！！！**
